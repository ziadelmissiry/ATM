...
public class Model 
{
    ...
    public void processBalance()
    {
        if (state.equals(LOGGED_IN) ) {
            number = 0;
            display2 = "Your balance is: " + bank.account.balance;
        } else {
            initialise("You are not logged in");
        }
        display();  // update the GUI
    ...
    //TDO process method for change password
    public void processChangePassword(){
        //check if the state is login
        if (state.equals(LOGGED_IN) ) {
            // go back to the log in state
            setState(ACCOUNT_NO);
            bank.changePassword(number);
            display2 = "Successfully changes password!\nKindly Login again";
            display1 = "";
            number=0;
            bank.logout();
        } else {
    ...
    //TDO
    //process method for overdraft
    public void processOverdraft()
    {
        //check if the state is login
        if (state.equals(LOGGED_IN) ) {  
            //overdraft from bank and display message
            if ( bank.overdraft(number ) )
            {
                display2 =   "Overrdraft: " + number;
            } else {
                display2 =   "Can't overdraft, you are exeeding the overdraft limit";
            }
            number = 0;
            display1 = "";           
        } else {
