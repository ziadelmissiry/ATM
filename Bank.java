...
public class Bank
{
    ...
    { 
        ...
        for(BankAccount bAccount:accounts){
            if(bAccount.accNumber==newAccNumber && bAccount.accPasswd==newAccPasswd){
                account = bAccount;
                return true;
            }
        }
        // TDO searching the array to find a bank account with matching account and password.
        // TDO not found - return false
        account = null;
        return false;
    }
    ...
    // TDO try to change password of an account (by calling the change password 
    // method on the BankAccount object) 
    public boolean changePassword(int password) 
    {
        if (loggedIn()) {
    ...
    //TDO try to overdraft money from the account (by calling the overdraft method
    // on the BankAccount object)
    public boolean overdraft(int amount) 
    {
        if (loggedIn()) {
            return account.overdraft(amount);   
        } else {
            return false;
        }
    }
